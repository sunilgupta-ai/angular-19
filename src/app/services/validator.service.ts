...
export class ValidatorService {
  ...
  isCharKey(event: any) {
    ...
    if (charCode2 > 32 && (charCode2 < 65 || charCode2 > 90) &&
      (charCode2 < 97 || charCode2 > 122) && charCode2 !== 46) {
      Swal.fire({
        icon: 'error',
  ...
  isAlphaNumeric(event: any) {
    ...
    // Allowed characters include alphanumeric, ',', '(', ')', and '_'
    if (
      ...
      Swal.fire({
        icon: 'error',
        text: 'Only Alpha Numeric Value Are Allowed'
      });
      return false;
    }
  }
